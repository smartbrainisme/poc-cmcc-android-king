...
public class AdapterMemberAll extends BaseAdapter implements
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(context).inflate(
					R.layout.listitem_member1, null);
			holder = new ViewHolder();
			holder.checkBox = (CheckBox) convertView
					.findViewById(R.id.talk_cb_group_member);
			holder.tvName = (TextView) convertView
					.findViewById(R.id.talk_tv_group_member);
			holder.ivSPresence = (ImageView) convertView
					.findViewById(R.id.talk_iv_presence);
			holder.ivRole = (ImageView) convertView
					.findViewById(R.id.talk_iv_group_role);
		...
		if (member != null) {
			...
			if (TextUtils.equals(AirtalkeeAccount.getInstance().getUserId(),
					member.getIpocId())) {
				holder.ivSPresence
						.setImageResource(R.drawable.user_state_online);
			} else {
				if (member.getStateInChat() == AirContact.IN_CHAT_STATE_ONLINE) {
					holder.ivSPresence
							.setImageResource(R.drawable.user_state_online);
				} else {
					int state = AirtalkeeContactPresence.getInstance()
							.getContactStateById(member.getIpocId());
					switch (state) {
					case AirContact.CONTACT_STATE_NONE:
						holder.ivSPresence
								.setImageResource(R.drawable.user_state_offline);
						break;
			...
			String myIpocId = (AirtalkeeAccount.getInstance() != null) ? AirtalkeeAccount
	...
	@Override
	public void onContactPresence(boolean isSubscribed,
			HashMap<String, Integer> presenceMap) {
		// TODO Auto-generated method stub
		notifyDataSetChanged();
	}
