...
//		mEditTextURI.setText(mPrefs.getString("uri", "rtsp://121.40.177.113:1935/live/test.stream"));//移动项目
//	    mEditTextUsername.setText(mPrefs.getString("username", "haolian"));//移动项目
//		mEditTextURI.setText(mPrefs.getString("uri", "rtsp://106.37.190.22:1935/live/test.stream"));//合纵思壮
	    ...
	    if(session != null)
	    {
	    	String sid = session.getSessionCode();
	    	if(session.getType() == AirSession.TYPE_CHANNEL)
	    	{
	    		sid = session.getSessionCode().substring(1, session.getSessionCode().length()); 
	    	}
	    	
	    	String uri ="rtsp://106.37.190.22:554/"+uid+"/"+session.getType()+"/"+sid+"/0";
	    	mEditTextURI.setText(uri);//合纵思壮
	    }
	    else
	    {
	    	Util.Toast(this, "当前会话未连接");
	    	finish();
	    }
	    */
