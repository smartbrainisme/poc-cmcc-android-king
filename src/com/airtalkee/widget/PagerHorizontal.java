 ...
 * a=blob;f=src/com/android/launcher/Workspace.java
 * 
...
public class PagerHorizontal extends ViewGroup
{
	...
	private int mAdjustedScrollX = 0;
	...
	private boolean mTouchAllow = true;
	 ...
	 *            values.
	 */
	public PagerHorizontal(Context context, AttributeSet attrs)
	{
	 ...
	 *            Unused.
	 */
	public PagerHorizontal(Context context, AttributeSet attrs, int defStyle)
	{
		super(context, attrs, defStyle);

		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.com_airtalkee_widget_PagerHorizontal);
		pageWidthSpec = a.getDimensionPixelSize(R.styleable.com_airtalkee_widget_PagerHorizontal_pageWidth, SPEC_UNDEFINED);
		a.recycle();
	...
	private void init()
	{
		...
		mTouchSlop = configuration.getScaledTouchSlop() + 30;
		mMaximumVelocity = configuration.getScaledMaximumFlingVelocity();
	}

	public void setPageSrcollAllow(boolean allow)
	{
		mTouchAllow = allow;
	}
	 ...
	 * the current page (and no other scrolling is occurring).
	...
	protected void dispatchDraw(Canvas canvas)
	{
		...
		try
		{
			...
			for (OnScrollListener mListener : mListeners)
			{
				...
				if (mAdjustedScrollX != adjustedScrollX)
				{
					...
					mAdjustedScrollX = adjustedScrollX;
				}
			}
		}
		catch (Exception e)
	...
	private void checkStartScroll(float x, float y)
	{
		...
		// Log.e("m",
		// String.format("xDiff =[%d]---mLastMotionX=[%s] floatX=[%s]",
		// xDiff,mLastMotionX+"",x+""));
		// Log.e("m",
		// String.format("yDiff =[%d]---mLastMotionY[%s] floatY=[%s]",
	...
	public static interface OnScrollListener
	{
		 ...
		 * assume the left edge of the first
		 * page has a scroll position of 0. Note that values less than 0 and
		 * greater than the right edge of the
